As stated in the [introduction](index.md), `eosiojs` integrates with EOSIO-based blockchains using the [EOSIO Nodala RPC API](https://developers.ala.io/eosio-nodeos/reference).

In general, there are two objects that are used to interact with a blockchain via `eosiojs`: the `JsonRpc` object, and the `Api` object.
...
The requests made by the `JsonRpc` object will either use a built-in `fetch` library, or [the `fetch` library passed in by the user](basic-usage/01_commonjs.md) to issue requests to the endpoint specified when instantiating the `JsonRpc` object.  When the various methods ([get_abi](https://github.com/EOSIO/eosiojs/blob/master/src/eosiojs-jsonrpc.ts#L66), [get_account](https://github.com/EOSIO/eosiojs/blob/master/src/eosiojs-jsonrpc.ts#L71), [get_block_header_state](https://github.com/EOSIO/eosiojs/blob/master/src/eosiojs-jsonrpc.ts#L76), etc) of the `JsonRpc` object are invoked, the calls are delegated to the `JsonRpc` object's [fetch function](https://github.com/EOSIO/eosiojs/blob/master/src/eosiojs-jsonrpc.ts#L42-L63), which in turn, delegate the requests to the `fetch` library.
...
The typical use of the `Api` object is to call its [`transact` method](https://github.com/EOSIO/eosiojs/blob/master/src/eosiojs-api.ts#L214-L254).  This method performs a number of steps depending on the input passed to it:

* The `transact` method first checks if the **chainId** was set in the `Api` constructor, and if not, uses the [`JsonRpc` object's](#jsonrpc) [`get_info`](https://github.com/EOSIO/eosiojs/blob/master/src/eosiojs-jsonrpc.ts#L101) method to retrieve the **chainId**.  
...
* The `actions` are serialized using the `eosiojs-serialize` `ser` object.
* The entire transaction is then [serialized](https://github.com/EOSIO/eosiojs/blob/master/src/eosiojs-api.ts#L154-L166), also using the `eosiojs-serialize` `ser` object.
...
* The transaction is then optionally broadcasted using `JsonRpc`'s [`push_transaction`](https://github.com/EOSIO/eosiojs/blob/master/src/eosiojs-jsonrpc.ts#L187).